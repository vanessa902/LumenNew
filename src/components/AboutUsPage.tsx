...
function TrackerCardView({ card }: { card: TrackerCard }) {
  return (
    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
      ...
      <div className="text-xs text-slate-500 mt-1">{card.source}</div>
      <div className="flex items-center justify-between mt-3">
        ...
        {card.days && (
          <span className="text-[10px] font-semibold text-white bg-blue-600 rounded-md px-2 py-1">{card.days}</span>
        )}
      </div>
    </div>
  )
    ...
    >
      <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-white text-slate-900">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center rounded-full border border-blue-200 overflow-hidden text-xs font-medium">
              <span className="flex items-center gap-1 px-3 py-1.5 bg-blue-600 text-white">
                <Sun className="w-3.5 h-3.5" />
                Light
              </span>
              <span className="flex items-center gap-1 px-3 py-1.5 text-slate-500">
                <Moon className="w-3.5 h-3.5" />
                Dark
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-9 h-9 rounded-full bg-slate-200 flex items-center justify-center text-slate-500">
                <User className="w-4 h-4" />
              </span>
              <div className="leading-tight">
                <div className="text-xs font-semibold text-slate-800">Luis Pérez</div>
                <div className="text-[10px] text-slate-400">Seller · Bluesun</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex">
          <div className="hidden md:flex w-14 flex-shrink-0 bg-blue-600 rounded-tr-[2rem] flex-col items-center gap-5 py-6">
            {SIDEBAR_ICONS.map((Icon, i) => (
              <span
                key={i}
                className={`w-8 h-8 rounded-full flex items-center justify-center ${i === 1 ? 'bg-white text-blue-600' : 'text-white/80'}`}
              >
          ...
          <div className="flex-1 p-6">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
              <h3 className="text-3xl font-extrabold tracking-tight text-slate-900">Install Tracker</h3>
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-slate-100 text-xs text-slate-400">
                  <Search className="w-3.5 h-3.5" />
                ...
                <button className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-slate-100 text-slate-600 text-xs font-semibold">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 overflow-x-auto">
              {TRACKER_COLUMNS.map(col => (
                <div key={col.name} className="bg-slate-50/60 rounded-xl p-3 min-h-[220px]">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    {col.count !== null && <span className="text-xs text-slate-400">{col.count}</span>}
                    <MoreVertical className="w-3.5 h-3.5 text-slate-300 ml-auto flex-shrink-0" />
                  </div>
                  {col.cards.length === 0 ? (
                    <div className="text-center text-xs text-slate-300 py-10">
